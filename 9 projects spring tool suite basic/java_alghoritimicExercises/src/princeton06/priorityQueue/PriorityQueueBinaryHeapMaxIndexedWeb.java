...
/**
 * The {@code IndexMaxPQ} class represents an indexed priority queue of generic
 ...
 * between {@code 0} and {@code maxN - 1} is associated with each key—the client
 * uses this integer to specify which key to delete or change. It also supports
 * methods for peeking at a maximum key, testing if the priority queue is empty,
 ...
 * <em>max-key</em>, <em>contains</em>, and <em>key-of</em> operations take
 ...
 */
public class PriorityQueueBinaryHeapMaxIndexedWeb<Key extends Comparable<Key>> implements Iterable<Integer> {
	 ...
	 * and {@code maxN - 1}.
	 *
	 * @param maxN the keys on this priority queue are index from {@code 0} to
	 *             {@code maxN - 1}
	 * @throws IllegalArgumentException if {@code maxN < 0}
	 */
	public PriorityQueueBinaryHeapMaxIndexedWeb(int maxN) {
		if (maxN < 0)
	...
	private class HeapIterator implements Iterator<Integer> {
		// create a new pq
		private PriorityQueueBinaryHeapMaxIndexedWeb<Key> copy;

		// add all elements to copy of heap
		// takes linear time since already in heap order so no keys move
		public HeapIterator() {
			copy = new PriorityQueueBinaryHeapMaxIndexedWeb<Key>(pq.length - 1);
			for (int i = 1; i <= n; i++)
	...
	public static void main(String[] args) {
		...
		PriorityQueueBinaryHeapMaxIndexedWeb<String> pq = new PriorityQueueBinaryHeapMaxIndexedWeb<String>(
				strings.length);
